...
def read_license_plate(license_plate_crop, min_score=0.45, debug=True):
    if license_plate_crop is None or license_plate_crop.size == 0:
        if debug:
            print("   âš ï¸ crop kosong / None")
        return None, None

    h, w = license_plate_crop.shape[:2]
    if h < 5 or w < 5:
        if debug:
            print(f"   âš ï¸ crop terlalu kecil: {w}x{h}")
        return None, None
    ...
    for vname, img in variants:
        try:
            ...
            if debug:
                print(f"   âŒ OCR exception [{vname}]: {e}")
            continue
        ...
        for box, (text, score) in result[0]:
            ...
            if debug:
                print(f"   ðŸ”¤ [{vname}] RAW='{raw}' | score={score:.3f} "
    ...
    if debug:
        print(f"   âž¡ï¸ HASIL AKHIR: {best_text} (score={best_score:.3f})")
